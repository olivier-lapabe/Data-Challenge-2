...
def main():
    """
    Re-train model with more complex data augmentation and higher regularization for in-depth training.
    """
    # Folder for model outputs
    log_directory = "./results/resnet101_intensive_training"
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), lr=0.0005, weight_decay=5e-4)  # Increased weight decay

    num_epochs = 150  # More epochs for deeper training

    # Cosine Annealing Learning Rate Scheduler
    scheduler = CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=0.00001)

    # Create Training, Validation dataloaders with intensive data augmentation
